...
function showNotification(title, message) {
    ...
    if (!container) return;
    // Toasts are addressed to whoever is driving. On a projector they land on
    // top of the product, so while the stage is the only thing on screen they
    // go to the diagnostics log instead of over the model.
    if (state.stageOnly) {
        addConsoleLog(`[${title}] ${message}`, 'info');
        return;
    }
